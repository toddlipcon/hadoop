...
public class TestNamenodePlugin {
  ...
  static {
    ...
    ((Log4JLogger) PluginBase.LOG).getLogger().setLevel(Level.ALL);
  ...
  public void testGetDatanodeReport() throws Exception {
    ...
    List<DatanodeInfo> nodes = namenode
        .getDatanodeReport(Constants.ALL_DATANODES);
    assertEquals(numNodes, nodes.size());

    nodes = namenode.getDatanodeReport(Constants.DEAD_DATANODES);
    assertEquals(0, nodes.size());

    nodes = namenode.getDatanodeReport(Constants.LIVE_DATANODES);
    assertEquals(numNodes, nodes.size());
  }
