...
interface DatanodeIf {
  ...
  public function writeBlock($block, $blockData);
}
...
class DatanodeClient implements DatanodeIf {
  ...
  public function writeBlock($block, $blockData)
  {
    $this->send_writeBlock($block, $blockData);
    $this->recv_writeBlock();
  }

  public function send_writeBlock($block, $blockData)
  {
    $args = new hadoop_api_Datanode_writeBlock_args();
    $args->block = $block;
    $args->blockData = $blockData;
    $bin_accel = ($this->output_ instanceof TProtocol::$TBINARYPROTOCOLACCELERATED) && function_exists('thrift_protocol_write_binary');
    if ($bin_accel)
    {
      thrift_protocol_write_binary($this->output_, 'writeBlock', TMessageType::CALL, $args, $this->seqid_, $this->output_->isStrictWrite());
    }
    else
    {
      $this->output_->writeMessageBegin('writeBlock', TMessageType::CALL, $this->seqid_);
      $args->write($this->output_);
      $this->output_->writeMessageEnd();
      $this->output_->getTransport()->flush();
    }
  ...
  public function recv_writeBlock()
  {
    $bin_accel = ($this->input_ instanceof TProtocol::$TBINARYPROTOCOLACCELERATED) && function_exists('thrift_protocol_read_binary');
    if ($bin_accel) $result = thrift_protocol_read_binary($this->input_, 'hadoop_api_Datanode_writeBlock_result', $this->input_->isStrictRead());
    else
    {
      $rseqid = 0;
      $fname = null;
      ...
      $result = new hadoop_api_Datanode_writeBlock_result();
      $result->read($this->input_);
      $this->input_->readMessageEnd();
    }
    if ($result->err !== null) {
...
class hadoop_api_Datanode_writeBlock_args {
  static $_TSPEC;
  ...
  public function __construct($vals=null) {
    ...
    if (is_array($vals)) {
      ...
      if (isset($vals['blockData'])) {
        $this->blockData = $vals['blockData'];
      }
    }
  }

  public function getName() {
    return 'Datanode_writeBlock_args';
  }
  ...
  public function read($input)
  {
    ...
    while (true)
    {
      ...
      switch ($fid)
      {
        ...
        case 2:
          if ($ftype == TType::STRUCT) {
            $this->blockData = new hadoop_api_BlockData();
            $xfer += $this->blockData->read($input);
          } else {
            $xfer += $input->skip($ftype);
          }
          break;
  ...
  public function write($output) {
    $xfer = 0;
    $xfer += $output->writeStructBegin('Datanode_writeBlock_args');
    if ($this->block !== null) {
      if (!is_object($this->block)) {
        throw new TProtocolException('Bad type in structure.', TProtocolException::INVALID_DATA);
      }
    ...
    if ($this->blockData !== null) {
      if (!is_object($this->blockData)) {
        throw new TProtocolException('Bad type in structure.', TProtocolException::INVALID_DATA);
      }
      $xfer += $output->writeFieldBegin('blockData', TType::STRUCT, 2);
      $xfer += $this->blockData->write($output);
      $xfer += $output->writeFieldEnd();
    }
    $xfer += $output->writeFieldStop();
    $xfer += $output->writeStructEnd();
...
class hadoop_api_Datanode_writeBlock_result {
  static $_TSPEC;
  ...
  public function getName() {
    return 'Datanode_writeBlock_result';
  }
  ...
  public function write($output) {
    $xfer = 0;
    $xfer += $output->writeStructBegin('Datanode_writeBlock_result');
    if ($this->err !== null) {
      $xfer += $output->writeFieldBegin('err', TType::STRUCT, 1);
      $xfer += $this->err->write($output);
      $xfer += $output->writeFieldEnd();
